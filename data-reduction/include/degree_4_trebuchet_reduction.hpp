...
#ifndef ALGORITHM_ENGINEERING_DEGREE_4_TREBUCHET_REDUCTION_HPP
#define ALGORITHM_ENGINEERING_DEGREE_4_TREBUCHET_REDUCTION_HPP

#include "data_reduction_node.hpp"

class degree_4_trebuchet_reduction : public data_reduction_node {
public:
    explicit degree_4_trebuchet_reduction (graph_unit graph);
...
private:
    ...
    void connect_to_them_except(graph_operation::stack &op_stack, vertex::id_type vertex,
                         const std::vector<vertex::id_type> &set, vertex::id_type except);


    static bool in_vc (vertex v);

    unsigned count_edges_in_subgraph(const vertex::id_type v, const std::vector<vertex::id_type>& subgraph);

};

#endif //ALGORITHM_ENGINEERING_DEGREE_4_TREBUCHET_REDUCTION_HPP